...
# get pathway/superpathway information
pw_superpw <- mum_pw_wide %>% select(path, path_2, super_pathway) %>% unique()

# Get pathways which are significant in both cohorts
sig_pw_both_chrt <- mum_pw_wide %>% 
  tidylog::filter(#sig == "Sig. Both Cohorts",
    pval_meta < 0.05,
    mixture == "all_pfas")

# Get pathways which are significant in either cohort
sig_either_chrt <- mum_pw_wide %>% 
  tidylog::filter(
    pval_meta < 0.05 | pval_sol < 0.05 | pval_chs < 0.05, # JG new 8/10/2022
    mixture == "all_pfas")
...
mwas_beta_coef_annotated_set <- mwas_beta_coef_w %>% 
  ...
  mutate(sig = case_when(p_value_sol_mixture   < 0.25 & 
                         ...
                         TRUE ~ "Not sig."), 
         bci_sig_sol = if_else(ucl_beta_sol_mixture <= 0 | 
                                 lcl_beta_sol_mixture >= 0,  
                               "Sig Sol", "Non-sig Sol"), 
         bci_sig_chs = if_else(ucl_beta_chs_mixture <= 0 | 
                                 lcl_beta_chs_mixture >= 0, 
                               "Sig CHS", "Non-sig CHS"), 
         bci_sig_pooled = if_else(ucl_beta_pooled_mixture <= 0 | 
...
mzrt_key_sig_pws_only_1 <- bind_cols(mzrt_key,
  ...
  unnest(pathway_dfs) %>% 
  # tidylog::filter(pathway_dfs %in% sig_pw_both_chrt$path) %>% #Original
  tidylog::filter(pathway_dfs %in% sig_either_chrt$path) %>% # JG change 08/10/2022
  ungroup() %>%
...
#                                   by = c("name" = "feature")) %>% 
#   tidylog::left_join(pw_superpw, by = c("pathway" = "path"))
# 
# 
...
mzrt_key_sig_pws_only_2 <- mzrt_key_sig_pws_only_1 %>% 
  ...
  tidylog::summarise(met_name = unique(met_name) %>% 
                     ...
                     pathway =  unique(pathway) %>% 
                       str_c(collapse = "; "),
                     super_pathway =  unique(super_pathway) %>% 
                       str_c(collapse = "; ")) %>% 
...
all_pfas_t1 <- t1 %>% 
  tidylog::filter(mixture == "all pfas")
...
summary_annotated_ee <- all_pfas_t1 %>% 
  ...
  tidylog::summarise(n_met_names = length(unique(met_name)), 
...
annotated_sig_ecs_ee <- all_pfas_t1 %>%  
  tidylog::filter(empirical_compound %in% 
                    sig_ecs$empirical_compound) %>% # New 08/10/2022 JG
                    # summary_annotated_ee$empirical_compound) %>% # Original
...
# Originally, 72 unique emperical compounds
# New data, August 15/2022 (with all pathways): 87 unique emperical compounds
length(unique(annotated_sig_ecs_ee$empirical_compound))
length(unique(annotated_sig_ecs_ee$name))
length(unique(annotated_sig_ecs_ee$super_pathway))
unique(annotated_sig_ecs_ee$matched_form)

# Save data
write_rds(annotated_sig_ecs_ee,
          here::here(dir_results_mixtures, 
                     "Sig annotated metabolite effect estimates sol chs_v4.RDS"))
# Version 4: Corrects issue where 2_4 was not run before saving the new data. 