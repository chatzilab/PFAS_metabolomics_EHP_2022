...
sol <- tyrosine_met$solar %>% 
  tidylog::select(met_name_tyr_pw:mass_diff, contains("sol")) %>% 
  mutate(cohort = "SOLAR") %>% 
  tidylog::rename_all(~str_remove(., "_sol"))



chs <- tyrosine_met$chs %>% 
...
ggsave(tyr_met_pips,
       filename = fs::path(
         dir_reports,
         "Figure S2 PFAS and tyr metabolite pips_R1.tiff"),
       width = 7.5, height = 9)
